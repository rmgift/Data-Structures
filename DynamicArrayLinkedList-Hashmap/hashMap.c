...
/* HASH_FUNCTION accepts a char* key and computes almost unique value based 
 * on the ASCII value of each letter in the key passed. Value returned from 
 * HASH_FUNCTION gets modulus with map capacity to get the int index of the 
 * linked list bucket
 */
...
void hashMapCleanUp(HashMap* map)
{
	...
	for (int i = 0; i < map->capacity; i++)
	{
		...
		/* while curr links exist in the index, delete it and go to next */
		while (cur != 0)
		{
...
int* hashMapGet(HashMap* map, const char* key)
{
	...
	/* while our current buck index still has list elements */
	while (bucketList != 0)
	{
...
void resizeTable(HashMap* map, int capacity)
{
	...
	/* clean up and remove old table from map but keep link to map */
	hashMapCleanUp(map);
...
{
	/* compute hash value to find the correct bucket and make sure we're dealing w/positive arith */
	int hashIndex = HASH_FUNCTION(key) % (map->capacity);
	if (hashIndex < 0)
	{
...
void hashMapRemove(HashMap* map, const char* key)
{
	...
	if (cur != 0)
	{
		/* if first index key matches passed key, set table index to nex element in list */
		...
		else
		{
			...
			/* found match, current holds link to remove, set prev to link beyond current */
			prev->next = cur->next;
		}
...
int hashMapContainsKey(HashMap* map, const char* key)
{
	...
	while (temp != 0) 
	{
		/* if the list element equals our test, found it */
