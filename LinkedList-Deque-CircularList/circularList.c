 ...
 * Name: Ryan Gift
 * Date: 04/30/2017
 * Description: This program implements the Deque ADT with a 
 * Circularly-Doubly-Linked List with 1 sentinel. The methods below add elements
 * to the list, remove elements, print the list, reverse the list, destroy the 
 * list, and indicate if the list is empty. This file should be combined with 
 * the circularList.h file and the circularListMain.c file.
 */
#include <stdio.h>
#include <stdlib.h>
#include <assert.h>
...
/* Adds a new link w/given value after the given link and incs the list's size. */
static void addLinkAfter(struct CircularList* list, struct Link* link, TYPE value)
{
	/* call createLink to create a link with the given value */
	struct Link *newLink = createLink(value);
	/* reassign next and prev based on link, then increment size */
...
static void removeLink(struct CircularList* list, struct Link* link)
{
	...
	/* reassign prev and next to remove link connection, free memory and dec size */
...
void circularListReverse(struct CircularList* list)
{
	...
	for (int i = 0; i < (list->size + 1); i++)
	{
		...
		/* set next and prev pointers to slide sentinel */
		temp->next = temp->prev;
		temp->prev = currData;
